...
const AppContent: React.FC = () => {
    ...
    const handleSaveAxis = (axisToSave: Axis, generatedScores: Record<string, number> | null, openScoringWizard?: boolean) => {
        if (!spaceData) return;
        let isNew = false;
    
        // First, apply any AI-generated scores to the styles
        let finalStyles = spaceData.styles;
        if (generatedScores) {
            finalStyles = spaceData.styles.map(style => {
                if (generatedScores[style.id] !== undefined) {
                    const newScores = { ...style.scores, [axisToSave.id]: generatedScores[style.id] };
        ...
        // Then, update the axes array
        const existing = spaceData.axes.find(a => a.id === axisToSave.id);
        let newAxes;
        if (existing) {
            newAxes = spaceData.axes.map(a => a.id === axisToSave.id ? axisToSave : a);
        } else {
            isNew = true;
            newAxes = [...spaceData.axes, axisToSave];
            // Also add to projection selections
            setUmapAxisIds(prev => [...prev, axisToSave.id]);
            setPcaAxisIds(prev => [...prev, axisToSave.id]);
        }
    
        const newSpaceData = { ...spaceData, axes: newAxes, styles: finalStyles };
        updateSpaceData(newSpaceData);
    
        setIsAxisModalOpen(false);
        setEditingAxis(null);
    
        // Open the scoring wizard if requested OR if it's a new axis without AI scores
        if (openScoringWizard || (isNew && !generatedScores)) {
    ...
    const openAxisModal = useCallback((axisId: string | null) => {
        ...
        if(axisId) {
            ...
            setEditingAxis({ id: uuidvv4(), name: 'New Axis', description: '', color: '#FFFFFF' });
        }
    ...
    const handlePointClick = useCallback((styleId: string | null) => {
